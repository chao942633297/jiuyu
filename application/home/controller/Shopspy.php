...
class Shopspy extends Controller
{
    ...
    public function spyadd()
    {
        ...
        if ($input['payment'] == 3) {
            ...
            Db::startTrans();
            try {
                // 直接扣除余额 生成窥探记录   是否中奖  中奖后抢购订单变更为失败并返钱 并更新商品信息
                ...
                $data = [];
                $data['sur_price'] = strval($insertData['sur_price']);
                ...
                Db::commit();
                return json(['code' => 1, 'data' => $data, 'msg' => '窥探成功']);
            } catch (\Exception $e) {
                Db::rollback();
    ...
    public function accountAdd($userid, $amount, $inset, $type, $remark = "窥探消费")
    {
        ...
        try {
            if ($inset == '1') {
                UserModel::get($userid)->setInc('balance', $amount);
            } else if ($inset == '2') {
                UserModel::get($userid)->setDec('balance', $amount);
            }
            // 增加用户余额消费记录
            ...
            $id = Db::name('account')->insertGetid($accountData);
            if ($id) {
                Db::commit();
                return $id;
            } else {
    ...
    public function setSpyingFail($goodsid, $times)
    {
        ...
        try {
            foreach ($faildata as $k => $v) {
                ...
                if ($r) {
                    $acId = $this->accountAdd($v['userid'], $v['sur_price'], '1', '13', $remark = "窥探抢购失败返还");
                    if ($acId < 0) {
                        Db::rollback();
                        return -4;
                    }
