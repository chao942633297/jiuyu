...
class Users extends Base
{
    ...
    public function editUser()
    {
        ...
        return json(['code' => 200, 'msg' => '查询成功', 'data' => $return]);
    }
    ...
    public function userSetting()
    {
        $user = UserModel::get($this->userId);
        $return = [];
        $return['alipay'] = $user['alipay']['alipay_account'];
    ...
    public function actBindAlipay(Request $request)
    {
        ...
        if (Db::table('sql_user_alipay')->where('alipay_account', $input['account'])->count() > 0) {
            return json(['msg' => '该支付宝账号已被绑定', 'code' => 1001]);
        }
        $user = UserModel::get($this->userId);
        if (md5($input['password']) !== $user['password']) {
            return json(['msg' => '登陆密码错误', 'code' => 1002]);
        }
        if (!empty($user['alipay']['alipay_account'])) {
            return json(['msg' => '您已绑定支付宝,请勿重复绑定', 'code' => 1002]);
        }
        $time = time() - 600;
     ...
     *支付宝解绑页面
     */
    public function webRelieveAlipay()
    {
        $user = UserModel::get($this->userId);
        $return = [];
        $return['alipay_account'] = $user['alipay']['alipay_account'];
        $return['alipay_name'] = $user['alipay']['alipay_name'];
        $return['phone'] = $user['phone'];
        return json(['data'=>$return,'msg'=>'查询成功','code'=>200]);
    }
     ...
     * 执行支付宝解绑
     */
    public function actRelieveAlipay(Request $request){
        $code = $request->param('code');
        $password = $request->param('password');
        $user = Db::table('sql_users')->where('id',$this->userId)->find();
        $time = time() - 600;
        $codeData = db('code')->where(['phone' => $user['phone'], 'type' => 6, 'status' => 1])->order('id', 'desc')->find();
        //TODO:获取验证码
        if ($user['phone'] != $codeData['phone'] || $code != $codeData['code']) {
            return json(['msg' => '验证码不正确', 'code' => 1002]);
        }
        if (strtotime($codeData['created_at']) < $time) {
            return json(['msg' => '验证码已失效,请重新获取', 'code' => 1010]);
        ...
        $result =  Db::table('sql_user_alipay')->where('user_id',$this->userId)->delete();
        if($result){
            return json(['msg'=>'解绑成功','code'=>200]);
        }
        return json(['msg'=>'解绑失败','code'=>1004]);
    }
...
/*    public function wechatBind()
    ...
    public function myQcode()
    {
        $user = UserModel::get($this->userId);
        if ($user['class'] < 2) {
            return json(['msg' => '需成为付费会员后才生成推广二维码', 'code' => 1001]);
        }
        $name = 'user_qcode_' . $user['unique'];
        $path = 'uploads/qcodeLogo/' . $name . '.png';     //的二维码
        if (!file_exists($path)) {
            $value = WxPayConf_pub::JS_API_BIND_URL.'?unique='.$user['unique'];
            $errorCorrectionLevel = "H"; // 纠错级别：L、M、Q、H
            $matrixPointSize = "4"; // 点的大小：1到10
            QRcode::png($value, $path, $errorCorrectionLevel, $matrixPointSize);
        }
